...
function php_saling(){
    ...
    // Ensure limit is set and is a valid number
    if (!isset($_SESSION['limit']) || !is_numeric($_SESSION['limit'])) {
        $_SESSION['limit'] = 10;  // Set a default limit if not set
    }
    ...
    if (!$result) {
        die("Query failed: " . mysqli_error($conn));  // Output the MySQL error for debugging
    }
